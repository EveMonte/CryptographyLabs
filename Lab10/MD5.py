...
class MD5:
    ...
    def third_step(self):
        ...
        for c in chunks:
            ...
            self.c0 += C
            self.d0 += D
        np_digest = np.array([str(hex(self.a0)).removeprefix('0x'), str(hex(self.b0)).removeprefix('0x'), str(hex(self.c0)).removeprefix('0x'), str(hex(self.d0)).removeprefix('0x')])
        return np_digest[0] + np_digest[1] + np_digest[2] + np_digest[3]